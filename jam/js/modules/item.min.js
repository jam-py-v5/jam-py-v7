...
return options;}
_do_open(offset,async,params,open_empty,callback){var self=this,i,filters,data;params=$.extend(true,{},params);if(params.__filters&&params.__filters.length){for(i=0;i<params.__filters.length;i++){let filter=params.__filters[i];if(filter[0]instanceof Array){params.__filters[i]=filter.map(function(or_filter){let item=or_filter.slice(0,3);while(item.length<3)item.push(null);return item;});}
else{let item=filter.slice(0,3);while(item.length<3)item.push(null);params.__filters[i]=item;}}}
